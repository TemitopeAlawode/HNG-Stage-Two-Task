...
exports.generateSummaryImage = generateSummaryImage;
const canvas_1 = require("canvas"); // Import functions to create an image canvas and register fonts
const path_1 = __importDefault(require("path")); // For working with file paths
const fs_1 = require("fs"); // For reading/writing files asynchronously
const Country_1 = __importDefault(require("../models/Country")); // Import the Country model for database operations
async function generateSummaryImage() {
    try {
        ...
        try {
            await fs_1.promises.access(fontPath); // Check if font file exists
            console.log('Font file exists');
            // Register multiple family names to handle mismatches
            (0, canvas_1.registerFont)(fontPath, { family: 'Open Sans' });
            (0, canvas_1.registerFont)(fontPath, { family: 'OpenSans' });
            (0, canvas_1.registerFont)(fontPath, { family: 'Open Sans Regular' });
        }
        catch (error) {
        ...
        // Fetch required data
        const total = await Country_1.default.count(); // Count total number of countries
        const top5 = await Country_1.default.findAll({
            order: [['estimated_gdp', 'DESC']],
            limit: 5,
            attributes: ['name', 'estimated_gdp'],
        });
        const lastRefresh = await Country_1.default.max('last_refreshed_at'); // Get last refresh timestamp
        ...
        const canvas = (0, canvas_1.createCanvas)(1200, 800);
